...
test.describe("Article API | CRUD operations", () => {
  test("should return a list of articles", async ({ api }) => {
  ...
  test("should create, update and delete an article", async ({ api }) => {
    ...
    const createArticleResponse = await api
      ...
      .body(articleRequest)
      .postRequest(201);
    await expect(createArticleResponse).shouldMatchSchema("articles", "POST_article_schema", false);
    ...
    expect(getArticleResponseAfterCreate.article.title).shouldBe(articleRequest.article.title);

    const updateArticleResponse = await api
      .path(`/articles/${slug}`)
      .body(articleRequest)
      .putRequest(200);
    await expect(updateArticleResponse).shouldMatchSchema("articles", "PUT_article_schema", false);
    expect(updateArticleResponse.article.title).shouldBe(articleRequest.article.title);
    ...
    expect(getArticleResponseAfterUpdate.article.title).shouldBe(articleRequest.article.title);
